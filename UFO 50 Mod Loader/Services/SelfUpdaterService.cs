        ...
        public static void CheckForUpdates(Window parent, bool automatic = false)
        {
            ...
            Dispatcher.UIThread.Post(async () =>
            {
                try
                {
                    ...
                    {
                        Logger.Log($"Downloading {Constants.ProgramName} update v{updateInfo.TargetFullRelease.Version}");
                        await updateManager.DownloadUpdatesAsync(updateInfo);

                        Logger.Log($"{Constants.ProgramName} update v{updateInfo.TargetFullRelease.Version} downloaded");
